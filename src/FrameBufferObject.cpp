...
void FrameBufferObject::AttachColorBuffer(const char* bufferName, GLenum attachment, int width, int height, int texIndex)
{
	GLuint colorBuffer;
...
void FrameBufferObject::AttachDepthBuffer(const char* bufferName, int width, int height, int texIndex)
{
	GLuint depthMap;
	glBindFramebuffer(GL_FRAMEBUFFER, mFrameBufferObject);
	glActiveTexture(GL_TEXTURE0 + texIndex);
