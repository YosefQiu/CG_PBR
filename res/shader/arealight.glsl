...
in vec3 PlanePos;
in vec3 lightDir;
in vec3 viewDir; // camPos-vertexPos
in float inside;
...
Ray GenerateCameraRay(vec3 CamPos)
{
    Ray ray;
    ...
    ray.origin = CamPos;
    ray.dir = normalize(teWorldPos - ray.origin);
...
void main()
{
    ...
    float distToRect;
    if (RayRectIntersect(ray, rect, distToRect)) // light object
